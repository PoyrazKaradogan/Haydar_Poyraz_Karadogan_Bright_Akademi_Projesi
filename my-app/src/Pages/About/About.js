import React from 'react'
import { Container } from 'react-bootstrap'
import './AboutPage.css'
const About = () => {
  return (
    <>
      <Container fluid className='d-flex flex-column justify-content-center align-items-center  page m-0 p-0 '>
        <Container>
<div className='m-5 text-light'
>
  <h3 className='m-2 text-danger'>Bahçeşehir Üniversitesi | Wissen</h3>
Wissen, Türkiye’nin en önemli eğitim kurumlarından olan Bahçeşehir Üniversitesi’ne bilgi teknolojileri ile ilgili sektörel eğitim programlarında içerik olarak danışmanlık veren bir kurumdur. Kurum kültürü olarak “eğitimden daha fazlası” sloganını kendine ilke edinmiş olan Wissen, Bahçeşehir Üniversitesi’nin akademik başarısıyla bu alandaki profesyonelliğini her yıl binlerce kişiye eğitim verdirerek sürdürmektedir. Bu sayede adayların, başarılı bir kariyer oluşturmasını sağlamaktadır. Bu zamana kadar verdiği danışmanlıkların ve şu anda da devam eden programlarının, Türkiye’nin teknolojik yatırımlarının geleceği ve başarısı açısından son derece önemli olduğunun farkında olarak eğitimlere devam edilmektedir.


Wissen’in amacı, bilişim teknolojileri ile ilgilenen adayları, bu alanda iyi eğitilmiş ve nitelikli kişiler haline getirmek ve bilişim firmalarını daha verimli bir seviyeye taşımaktır. Bu sayede, ülkemizin üreteceği teknolojilerin niteliği artacak ve katma değeri yüksek işler ortaya çıkacaktır. Wissen, bu amaçla, uluslararası anlaşmalarını arttırarak Microsoft ile olan ilişkilerini Gold Partner seviyesine çıkarmış, People Cert, Autodesk, Samsung, Amazon, Cisco, gibi kurumlarla partnerlikler oluşturmuştur ve partnerliklerini aşağıdaki gibi detaylandırmıştır;


Microsoft: Microsoft Gold Learning Partneri olan Wissen, şu anda Türkiye’deki iki gold partnerden biri olarak Microsoft’la olan çözüm ortaklığına devam etmektedir. Ayrıca Dynamics CRM ve AX tarafında da yetkili learning partner olmuştur.

Autodesk: Yetkili eğitim merkezi ve yetkili sınav merkezi olarak eğitimlerine devam etmektedir.

Amazon Web Services: Amazon sertifikalı eğitmenleri ile yetkili eğitim merkezi olarak, firmalara bulut bilişim altyapılarının eğitimini vermektedir.

Android ATC: İstanbul’daki tek yetkili eğitim ve sınav merkezi olarak, Samsung işbirliği ile kurulmuş laboratuvarları ile Android eğitimine devam etmektedir..

People Cert (ITIL): Yetkili eğitim ve sınav merkezi olarak çalışmalarına devam etmektedir.

Toon Boom: Türkiye’deki tek yetkili eğitim merkezi olarak animasyon eğitimlerine devam etmektedir.

CompTIA: Yetkili eğitim merkezi olarak çalışmalarına devam etmektedir.

Bütün bu bilgiler ve profesyonel hayatın ihtiyaçları doğrultusunda Wissen , aşağıdaki dört alanda uzmanlaşmıştır;


Nitelikli Bilgi Teknolojileri Uzmanı Yetiştirme Programı
Bahçeşehir Üniversitesi ve Çalışma ve Sosyal Güvenlik Bakanlığı iş birliği ile oluşturulan, %100 devlet finansmanlı oluşturulan bilgi teknolojileri uzmanlığı eğitimleridir. Bu eğitimler ile sayısal ilgili lisans mezunu çalışmayan adaylar, %100 burslu olarak , Java Programlama ve Veritabanı Eğitimi, Microsoft Yazılım ve Veritabanı Eğitimi, Microsoft Sistem ve Network Eğitimi, Cisco Network Eğitimeleri, Microsoft Dynamics ile ERP Eğitimleri alırlar. Katılımcıların eğitim boyunca genel sağlık sigortası devlet tarafından karşılanmaktadır. Ayrıca katılımcılar eğitim süresince maddi destek alarak, eğitim sonunda katılımcıların en az %60’ı istihdam edilmektedir. Bu eğitimlerle katılımcılar, alanlarında teknik açıdan uzmanlaşırlar. Eğitim aldıkları alanda proje geliştirme, sektör bilinci, iş bulma yetkinliği gibi konularda gelişim şansı yakalarlar. Ayrıca katılımcılara ücretsiz kariyer danışmanlığı sağlanarak iş görüşmesi deneyimi sağlanır ve eğitim sonunda işe yönlendirmeleri sağlanır.

<h3 className='m-2 text-danger' >Açık Sınıf Eğitimleri</h3>



Kendileri için daha iyi bir iş perspektifi oluşturmak isteyen adaylar için düzenlenen bu eğitimlerle kişiler, seçtikleri alanda uzmanlıklarını tamamlarlar. Bu kısımda Wissen, aşağıdaki alanlarda yılda ortalama 2000’in üzerinde kişiye eğitim danışmanlığı vermektedir;


Introduction to Programming in C#
Implementing Entity Framework With MSSQL
Asp.Net MVC 5 Web Application Development
Asp.Net WebForm Application Development
Network Fundamentals and Windows 10 with Client Systems
Microsoft Windows Server 2016
Identity with Windows Server 2016 Security
Microsoft Exchange Server 2016
Java Standard Edition
Java Enterprise Edition
IOS Programming with Swift
Android Programming
Autocad
3D Studio Max
Revit Architecture
Amazon Web Services Technical Essentials (AWS)
Architecting on Amazon Web Services (AWS)
System Operations on AWS
Google Cloud Platform: Fundamentals
Kurumsal Eğitimler
Wissen, büyük ve orta ölçekli firmalara, firmaların hedefleri ve ihtiyaçları doğrultusunda düzenlenen bu eğitimlerle, çalışanlar kendilerini değişen teknolojik gelişmelere adapte ederler. Bu kısımda Wissen; Can Sigorta, İstanbul Barosu, Danfoss, Ulaşım A.Ş, TEB, Albaraka Türk, Türk Telekom, İstanbul Şehir Hatları, Akbank, Süleyman Demirel Üniversitesi, HEGEM, PAVOTEK, Türkiye Turing ve Otomobil Kurumu, Doğuş Otomotiv, ARÇELİK, Turpak A. Ş., Türk Telekomünikasyon A.Ş., Türkiye Turing ve Otomobil Kurumu, ARKAN ERGİN, THY DO CO, Çaykur, İTKİB, Berner A.Ş, Garanti Bankası, Obase Bilgisayar, Analitik Bilişim, ETG, Hitachi, Microsoft, Softtech, Özyeğin Üniversitesi, Supplementler.com gibi firmalara kurumsal eğitim danışmanlıkları vermektedir.

<h3  className='m-2 text-danger' >CO-OP Eğitimleri</h3>
Bahçeşehir Üniversitesi Mühendislik ve Doğa Bilimleri Fakültesi’nde okuyan adaylar için CO-OP Direktörlüğü iş birliği ile düzenlenen bu eğitimler ile öğrenciler, Android, iOS, Bilgi Güvenliği gibi alanlarda kendilerini geliştirme imkanı bulurlar. Bu kısımda Wissen, aşağıdaki alanlarda yılda ortalama 700 – 1000 kişiye eğitim danışmanlığı vermektedir;


Advanced Developing Application for Android Devices
Cloud Computing and its Applications
Advanced Mobile Operating Systems& App.
Asp.Net MVC
Advanced Network Systems Engineering
Cross Platform Development
Front End Development
Cybersecurity Information Systems Security
3D Modelling & Designc
Mobile App. Development for Windows Phone

Wissen, son yıllarda bilişimin önde gelen kurumlarıyla üst düzey işbirlikleri yapmıştır. Amazon’un Türkiye’de ilk olarak learning partneri olan Wissen, web hizmetleri ve bulut bilişim, big data, bilgi güvenliği gibi yeni alanlarda da sunduğu danışmanlıklarla Türkiye’de de bir çok kuruma öncü olmuştur. Son teknoloji donanımıyla desteklenmiş Apple iSchool ve Samsung işbirliği ile kurduğu Android laboratuvarlarıyla genç uygulama geliştiricilere benzersiz bir kuluçka merkezi ortamı sağlamıştır. Wissen’den aldığı eğitimlerle kendi firmasını kurarak başarı hikayesi oluşturmuş birçok aday ile girişimciler için aranan bir merkez haline gelmiştir. Ayrıca bilişim eğitimlerinde TEB,Türk Telekom, Akbank, Turpak, Çaykur ,THY Do-Co, İstanbul Şehir Hatları ,İstanbul Ulaşım AŞ., Ziraat Bankası, Koç Sistemvb. gibi büyük şirketlerle kurulan sürdürülebilir işbirlikleri, Wissen’ in kurumsal eğitim danışmalığı iddiasını hayata geçirmektedir.


Uzun vadede, tüm dünyada gerçekleşen dijital dönüşümle beraber, genç adaylar ile çalışan nüfusu bugün var olmayan, henüz ismini bile duymadığımız, fakat zamanla ihtiyaç haline gelecek meslek gruplarına hazırlamalı ve bu kişileri global problemleri çözecek teknolojileri ilk olarak geliştirecek profesyoneller olarak yetiştirmeliyiz.


Wissen’in yönetim kadrosu ve danışmanları olarak, daha üretken ve etkin kurumlar oluşturabilecek, iyi eğitimli profesyoneller yaratabilmek amacıyla çalışmalarımıza durmaksızın devam etmekteyiz.</div>
    </Container>  
    </Container>
    </>
  )
}

export default About